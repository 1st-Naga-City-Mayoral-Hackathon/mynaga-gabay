...
export interface SymptomContext {
  duration?: 'acute' | 'prolonged'; // acute: < 7 days, prolonged: >= 7 days
  fever?: boolean;
...
export type TriageLanguage = 'english' | 'tagalog' | 'bikol';
...
// Red flag patterns that indicate emergency
const RED_FLAG_PATTERNS: Record<string, { pattern: RegExp; message: string }[]> = {
  cough: [
    ...
    { pattern: /\b(high.*fever|mataas.*lagnat|39|40|41)\b/i, message: 'High fever (39°C or above)' },
    { pattern: /\b(more.*than.*2.*weeks|2\+.*weeks|matagal.*na|ilang.*linggo)\b/i, message: 'Cough lasting more than 2 weeks' },
  ],
  fever: [
    { pattern: /\b(very.*high|mataas.*mataas|40|41|42)\b/i, message: 'Very high fever (40°C or above)' },
...
const OTC_MEDICATIONS: Record<string, MedicationCardItem[]> = {
  cough: [
    {
  ...
  fever: [
    {
      ...
      whenToSeeDoctor: 'If fever exceeds 39°C, lasts more than 3 days, or is accompanied by severe symptoms',
    },
    {
 ...
 */
export function detectSymptoms(message: string): SymptomMatch[] {
...
export function detectRedFlags(
  message: string,
  symptoms: string[]
): { flags: string[]; requiresER: boolean } {
  const flags: string[] = [];
  ...
  const requiresER = flags.some((flag) =>
    erIndicators.some((indicator) => flag.toLowerCase().includes(indicator.toLowerCase()))
  );
 ...
 */
export function getMedicationCard(symptoms: string[]): MedicationCard | undefined {
  const items: MedicationCardItem[] = [];

  for (const symptom of symptoms) {
    const meds = OTC_MEDICATIONS[symptom];
    if (meds) {
      // Add first 2 medications for each symptom
  ...
  return {
    ...
    items,
    generalDisclaimer:
      'This is general information only. Always read the label carefully, follow dosage instructions, ' +
      'and consult a pharmacist or doctor before taking any medication, especially if you have other ' +
 ...
 */
export function getFollowUpQuestionsLocalized(
  symptoms: string[],
...
export function triageMessage(
  message: string,
  language: TriageLanguage = 'tagalog'
): TriageResult {
  // Detect symptoms
  ...
  // Get medication recommendations (only for self-care)
  const medicationCard =
    urgency === 'self_care' ? getMedicationCard(detectedSymptoms) : undefined;

  // Get follow-up questions
  const followUpQuestions =
    detectedSymptoms.length > 0 && urgency !== 'er'
      ? getFollowUpQuestionsLocalized(detectedSymptoms, language)
      : undefined;

  // Determine facility type
  let facilityType: TriageResult['facilityType'] = 'none';
  if (requiresER) {
    facilityType = 'er';
...
/**
 * Build appropriate disclaimer based on urgency
 */
function buildDisclaimer(
  ...
  if (urgency === 'er') {
    if (language === 'english') {
      return (
        'WARNING: Your symptoms may be serious. Please go to the nearest emergency room immediately or call emergency services.'
      );
    }
    if (language === 'bikol') {
      return (
        'BABALA: Pwedeng seryoso an imo mga sintomas. Paki-adto sa pinakamalapit na emergency room ngunyan o tumawag sa emergency services.'
      );
    }
    return (
      'BABALA: Ang iyong mga sintomas ay maaaring seryoso. Mangyaring pumunta sa pinakamalapit na emergency room kaagad o tumawag sa emergency services.'
    );
  }
  ...
  if (hasRedFlags) {
    if (language === 'english') {
      return (
        'Important: Some symptoms may need medical attention. We recommend consulting a healthcare professional soon.'
      );
    }
    if (language === 'bikol') {
      return (
        'Importante: May mga sintomas na pwedeng kailangang makita nin doktor. Irekomenda namon na kumonsulta sa healthcare professional sa pinakadali.'
      );
    }
    return (
      'Mahalaga: May ilang sintomas na maaaring kailangang tingnan ng doktor. Inirerekomenda naming kumonsulta sa isang healthcare professional sa lalong madaling panahon.'
    );
  }

  if (language === 'english') {
    return (
      'Reminder: This information is general guidance only and not a substitute for professional medical advice. Consult a doctor or pharmacist if unsure.'
    );
  }
  if (language === 'bikol') {
    return (
      'Pahiling: Ini pangkalahatang gabay sana, bako kapalit nin propesyonal na medikal na payo. Kumonsulta sa doktor o pharmacist kun dae ka sigurado.'
    );
  }
  return (
    'Paalala: Ang impormasyong ito ay pangkalahatang gabay lamang at hindi kapalit ng propesyonal na medikal na payo. Kumonsulta sa doktor o pharmacist kung hindi ka sigurado.'
  );
}
